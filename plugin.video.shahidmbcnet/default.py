...
def Addtypes():
	...
	addDir('Channels' ,'http://shahid.mbc.net/ar/channel-browser.html' ,2,'') #links #2 channels,3 series,4 video entry, 5 play
	addDir('All Series' ,'http://shahid.mbc.net/ar/series-browser.html' ,6,'')
...
#	match =re.findall('<img src="(.*?)" alt=".*".+<\/a>\n*.+<div class="post-title"><a href="(.*?)".*<b>(.*)<\/b>', link, re.UNICODE)
	...
	if mode==6:
		...
		purl='http://shahid.mbc.net/ar/series-browser/autoGeneratedContent/seriesBrowserGrid~browse~-param-.sort-latest.pageNumber-%s.html' % pageNumber
...
#	print url
	...
	paymentID=match[0][1]
	playlistURL="http://shahid.mbc.net/arContent/getPlayerContent-param-.id-%s.type-player.pricingPlanId-%s.html" % ( videoID,paymentID)
